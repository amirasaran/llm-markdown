...
export function CalloutDom({ attributes, children, theme }: DirectiveComponentProps) {
  ...
  return (
    ...
    >
      ...
      />
      <div style={{ flex: 1, color: palette.fg }}>
        {title ? (
          <div style={{ fontWeight: 700, color: palette.fg, marginBottom: 4 }}>{title}</div>
        ) : null}
        <div>{children}</div>
      </div>
    </div>
